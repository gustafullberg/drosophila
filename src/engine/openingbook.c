...
static const uint64_t *RandomPiece = Random64+0;
static const uint64_t *RandomCastle = Random64+768;
static const uint64_t *RandomEnPassant = Random64+772;
    ...
    for(color = WHITE; color <= BLACK; color++) {
        for(type = PAWN; type <= KING; type++) {
            ...
            while(pieces) {
                ...
                int offset_piece = 64*kind_of_piece + pos;

                hash ^= RandomPiece[offset_piece];
